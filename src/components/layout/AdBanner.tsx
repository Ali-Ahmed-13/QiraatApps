...
export default function AdBanner({ position }: AdBannerProps) {
  ...
  return (
    <div
      className={`w-full mx-auto my-6 p-4 rounded-2xl border border-slate-200/60 dark:border-slate-800/80 bg-white/40 dark:bg-slate-900/40 backdrop-blur-sm flex flex-col items-center justify-center text-center relative overflow-hidden transition-colors duration-300 ${
    ...
    >
      ...
      {/* Ad Label Tag */}
      <span className="absolute top-2 left-3 px-2 py-0.5 text-[10px] font-bold text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-800/60 rounded-md">
        مساحة إعلانية
      </span>

      <div className="flex flex-col sm:flex-row items-center gap-3 text-slate-400 dark:text-slate-500">
        <Megaphone className="w-5 h-5 text-emerald-600/50 dark:text-emerald-400/40" />
        <div>
